...
export default function FortuneResult({ fortune, previewUrl, readingsLeft, onReset }: Props) {
    ...
    <div className="min-h-screen flex flex-col items-center px-4 py-14">
      ...
      >
        <p className="text-white/85 text-lg leading-relaxed whitespace-pre-line font-light">
          {displayed}
          {typing && (
            <span className="inline-block w-0.5 h-5 bg-gold ml-0.5 align-middle animate-pulse" />
          )}
        </p>
        {typing && (
          <p className="text-white/20 text-xs mt-4 text-right">Devamını görmek için dokun...</p>
        )}
      </div>
